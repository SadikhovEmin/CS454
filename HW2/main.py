...
with open('testing.csv') as testing_file:
        ...
        for j in training_csv:
            temp_results.append([math.sqrt(
                math.pow((float(i[0]) - float(j[0])), 2) + math.pow((float(i[1]) - float(j[1])),
                                                                    2)), training_csv.index(j)])
        # print(sorted(temp_results))
        list_euclidean.append(sorted(temp_results))  # Sorting all of the euclidean

    # Need to calculate count of them for k = 1, 3, 5, 7, 9 and plot them
    # First creating a confusion matrix
    # confusion_matrix = [
    #     [0, 0, 0],
    #     [0, 0, 0],
    #     [0, 0, 0]
    ...
    print(testing_csv[0])
    print(list_euclidean[0], sep='\n')
    count, k = 0, 1
    '''
    i = testingdeki bir instance
    list_euclidean[index(i)]
    
    '''
    print(len(list_euclidean[0]))
    for i in list_euclidean:
        ...
        for j in range(0, k):
            if list_euclidean[j][2] == "Iris-setosa":
                setosa += 1
            elif list_euclidean[j][2] == "Iris-versicolor":
                versicolor += 1
            elif list_euclidean[j][2] == "Iris-virginica":
                virginica += 1

        for z in testing_csv:
            if virginica > setosa and virginica > versicolor:
                if z[2] == "Iris-setosa":
                    confusion_matrix[2][0] += 1
                if z[2] == "Iris-versicolor":
                    confusion_matrix[2][1] += 1
                if z[2] == "Iris-virginica":
                    confusion_matrix[2][2] += 1
